...
function initRouter() {
  ...
  return new Promise(resolve => {
    getAsyncRoutes().then(({ data }) => {
      const userRoles = getRulesTree(data, 0, []);
      parantArr = userRoles;
      const result = TreeList(userRoles, []);
...
export function getRulesTree(rulesList, id, list) {
...
let parantArr = [];
function TreeList(data, arr) {
  data.forEach((datas, index) => {
    arr.push({
      path: datas.pid === 0 ? "/" + datas.menu_url : "/" + findParentPath(datas.pid) + "/" + datas.menu_url,
      name: datas.menu_name,
      component: datas.menu_url,
      meta: {
        title: datas.menu_name,
        icon: datas.pid === 0 ? datas.icon : "",
        showParent: true
      },
      pid: datas.pid,
      id: datas.id,
    ...
    if (datas.children) {
      const childArr = TreeList(datas.children, []);
      arr[index].children = childArr;
    }
    if (arr[index].children == 0) {
...
function findParentPath(pid) {
  if (!pid) return "";
  let res = parantArr.filter(item => item.id === pid);
  return res[0]?.menu_url;
}
...
function addAsyncRoutes(arrRoutes: Array<RouteRecordRaw>) {
  ...
  /** 遍历全部  去除components下的index*/
  const modulesRoutesKeys = Object.keys(modulesRoutes).filter(item => {
  ...
  arrRoutes.forEach((v: RouteRecordRaw) => {
    ...
    if (v.meta?.frameSrc) {
      ...
      // 修改后
      const regex = new RegExp(`/${v.component}/`);
      const index = v?.component ? modulesRoutesKeys.findIndex(ev => regex.test(ev)) : modulesRoutesKeys.findIndex(ev => ev.includes(v.path));
