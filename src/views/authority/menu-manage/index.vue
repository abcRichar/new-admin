  ...
  <div class="main">
    <el-form ref="formRef" :inline="true" :model="form" class="search-form bg-bg_color w-[99/100] pl-8 pt-[12px] overflow-auto">
      <el-form-item label="菜单名称：" prop="menu_name">
        <el-input v-model="form.menu_name" placeholder="请输入菜单名称" clearable class="!w-[180px]" />
      </el-form-item>
      <el-form-item>
        ...
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            <template #operation="{ row }">
              <el-button class="reset-margin" type="primary" :size="size" :icon="useRenderIcon(EditPen)" @click="openDialog('编辑', row)"> 编辑 </el-button>
              <el-button class="reset-margin" type="primary" :size="size" :icon="useRenderIcon(Delete)" @click="handleDelete(row)"> 删除 </el-button>
            </template>
          </pure-table>
        </template>
      </PureTableBar>
